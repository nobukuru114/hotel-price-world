...
const files = fs.existsSync(PEND) ? fs.readdirSync(PEND).filter(f => f.endsWith(".json")) : [];
if(!files.length){ console.log("pending なし"); process.exit(0); }
const KEYS = ["name","c","med","lo","hi","cnt","lat","lng","m","ap","ja"];
for(const f of files){
  ...
  const rec = Object.fromEntries(KEYS.map(k => [k, r[k]]));
  const i = DATA.findIndex(d => d.name === rec.name && d.c === rec.c);
  if(i >= 0){ Object.assign(DATA[i], { med: rec.med, lo: rec.lo, hi: rec.hi, cnt: rec.cnt, m: rec.m, ap: rec.ap }); console.log("更新:", rec.name); }
